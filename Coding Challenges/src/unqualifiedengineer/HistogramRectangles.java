...
public final class HistogramRectangles {
    ...
    * 
    * If the given array of int is null, throws NullPointerException, and if any
    * int contained by the array of int is negative, throws
    * IllegalArgumentException.
    * 
    * @param histogram
    *        array representation where indexes are the x-axis positions and int
    *        values are the y-axis heights
   ...
   public static int findLargestRectangle(int[] histogram) {
      ...
      Int height = new Int(0), position = new Int(0), tempHeight = new Int(0),
            tempPosition = new Int(0), tempSize = new Int(0),
            maxSize = new Int(Integer.MIN_VALUE);
      ...
      for (; position.i < histogram.length; position.i++) {
         ...
         if (heights.isEmpty() || height.i > heights.peek().i) {
            ...
            // current height in the stack is less than the current height that
            // we care about

            // regardless of position, return those values to the stack
            heights.push(new Int(height.i));
            positions.push(new Int(tempPosition.i)); // not position!
            // this push allows us to "look back in time" at older rectangles
         }
      }
