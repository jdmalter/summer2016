...
public final class CalendarConflicts {
	...
	public static final class Event {
		...
		/**
		 * Checks argument validity and sets fields to the respective
		 * parameters.
		 * 
		...
		public Event(String id, int start, int end) {
			if (start >= end)
				throw new IllegalArgumentException("start must not be greater than or equal to end");
	...
	public List<String> findConflicts() {
		...
		if (calendar.length == 0 || calendar.length == 1)
			return conflicts;
		...
		for (int i = 1; i < calendar.length; i++) {
			...
			if (calendar[i].start >= end) { // no conflict
				...
				// flush every temporary conflict to final conflict list since
				// no
				// conflicts will combine at this point in code, also there must
				// be
				// more than one id to create a conflict (atleast two events in
				// a
				// conflict)
				if (temporary.size() > 1)
			...
			// use the maximum time because the lastest starting event may not
			// have
