...
public final class IntPermutationGenerator {
   ...
   public IntPermutationGenerator(int size) {
      if (size <= 1) throw new IllegalArgumentException(
         "size must not be less than or equal to one");

      // zeros cannot exist in any return from this permutation generator since
   ...
   public int[] generate() {
      ...
      if (base[0] == 0) return fill();

      int decrease = base.length - 2; // furthest right position of decrease
      // partition is compared to the element greater than itself, therefore the
      ...
      // loop finds any decrease of two numbers in base array unless the numbers
      // are reverse sorted like n, n - 1, n - 2, ... 1, but if the numbers are
      // reversed sorted as given, every other permutation must have already
      // been generated
      while (base[decrease] > base[decrease + 1] && decrease >= 0) {
         if (decrease == 0) throw new IllegalStateException(
