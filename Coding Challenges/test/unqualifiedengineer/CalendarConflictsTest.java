...
public class CalendarConflictsTest {
   ...
   private Event[] calendar = new Event[] { new Event("a", 1, 2),
      new Event("b", 3, 5), new Event("c", 4, 6), new Event("d", 7, 10),
      new Event("e", 8, 11), new Event("f", 10, 12), new Event("g", 13, 14),
      new Event("h", 13, 14) };
   ...
   public void testFindConflicts() {
      assertEquals(0,
         new CalendarConflicts(new Event[] {}).findConflicts().size());

      assertEquals(0,
         new CalendarConflicts(new Event[] { new Event("placeholder", 0, 1) })
            .findConflicts().size());
   ...
   private static void equals(List<String> expected, List<String> actual) {
      ...
      for (Iterator<String> eIterator = expected.iterator(), aIterator =
         actual.iterator(); eIterator.hasNext();) {
